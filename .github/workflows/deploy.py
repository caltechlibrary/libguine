    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--login":
        with sync_playwright() as p:
            ...
            if os.environ.get("DEBUG") == "true":
                context = browser.new_context(base_url=os.environ.get("ADMIN_BASE_URL").rstrip("/"), headless=True, slow_mo=100, record_video_dir="debug")
            else:
            ...
            try:
                ...
                page.wait_for_selector("#s-libapps-code")
                ...
                for item in os.scandir("artifacts"):
                    ...
                    if item.name.endswith(".css") or item.name.endswith(".js"):
                        ...
                        if target == "template":
                            ...
                            page.fill("#name", item.name.split("-", maxsplit=2)[-1].split(".")[0])
                            page.click("#lg-admin-asset-filter .datatable-filter__button--submit")
                            try:
                            ...
                            if variant == "system":
                                ...
                                for group in json.loads(os.environ.get("GROUPS"))["groups"]:
                                    if variant == group["slug"]:
                                        ...
                                        page.click("#s-lg-btn-save-footer")
                                        # NOTE must wait for success before moving on
                                        page.wait_for_selector("#s-lg-btn-save-footer.btn-success")
            finally:
                context.close()
                browser.close()
